...
class IngenieroController extends Controller
{
    public function index()
    {
        $data = DB::table('trabajador')
            ...
            ->get();
        return view ('ingeniero.index',['data'=>$data]);
    }
    public function create()
    {
        return view('ingeniero.create');
    }
    public function store(Request $request)
    {
        ...
        return redirect()->route('ingeniero.index')->with('status', 'Ingeniero agregado(a) correctamente!');
    }
    ...
    public function destroy($id)
    {
        ...
        if($trab->trab_est == 1){
            $trab->trab_est = 0;
            $trab->save();
            return redirect()->route('ingeniero.index')->with('status', 'Se cambio el estado del Ingeniero Inactivo');
        } else {
            $trab->trab_est = 1;
            $trab->save();
            return redirect()->route('ingeniero.index')->with('status', 'Se cambio el estado del Ingeniero Activo');
        }
    }
}
